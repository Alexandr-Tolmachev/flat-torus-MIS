...
class IntelTreesearch(MWISSolver):
    ...
    def _prepare_instance(source_instance_file: pathlib.Path, cache_directory: pathlib.Path, **kwargs):
        ...
        # added
        edges_from = np.array([item[0] for item in g.edges])
        edges_to = np.array([item[1] for item in g.edges])
        g = dgl.graph((np.concatenate((edges_from, edges_to)), np.concatenate((edges_to, edges_from))))
        #
        
        #g = dgl.from_networkx(g, node_attrs=["label"] if labels_given else [])
        
        print("Graph converted to the dgl format!")
    ...
    def solve(self, solve_data_path: pathlib.Path, results_path: pathlib.Path, parameters):
        ...
        if "without_result_overwrite" in parameters.keys():
            arguments += ["--without_result_overwrite"]
