...
class KaMIS(MWISSolver):
    ...
    def solve(self, solve_data_path: pathlib.Path, results_path: pathlib.Path, parameters):
        ...
        for graph_path in solve_data_path.rglob("*.gpickle"):
            ...
            results_filename.unlink()  # delete temporary file

        # save results
        without_result_overwrite = "without_result_overwrite" in parameters.keys()
        write_regime = 'a' if without_result_overwrite else 'w'
            
        with open(results_path / "results.json", write_regime, encoding='utf-8') as f:
